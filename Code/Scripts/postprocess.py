...
for l in range(len(varList)):
    ...
    ax.set_title("Channel flow: RANS, k-omega",fontsize=15)
    ax.set_xlabel(r'$y/\delta$',fontsize=8)
    ax.set_ylabel(r'$log_{10}(Re_b)$',fontsize=8)
    ax.set_zlabel(f"{varList[l]}")#r'$u/U_b$',fontsize=8)
    ax.tick_params(axis='both', which='major', labelsize=6)
    ax.tick_params(axis='both', which='minor', labelsize=6)
    elevVar=np.array([  0, 20,20,20, 0,90,45,0])
    azimVar=np.array([-90,-45, 0,45,90, 0,45,0])
    for i in range(elevVar.size):
        ax.view_init(elev=elevVar[i], azim=azimVar[i])
        fig.savefig(f"K_{varList[l]}_ResponseSurf_{elevVar[i]}_{azimVar[i]}.png",dpi=250)
        
    f=open(f"K_{varList[l]}_channelFlow_ResponseSurf_Data","w")
    f.write("y \t Re \t u \n")
    for i in range(x.size):
        f.write(f"{format(x[i],'.3E')} \t {format(Re[i],'.3E')} \t {format(u[i],'.3E')} \n")
    f.close()
    plt.show()
